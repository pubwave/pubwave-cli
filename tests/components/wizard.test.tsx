...
describe("SetupWizard local-model navigation (Fix #1)", () => {
  it("renders the first (language) step", async () => {
  ...
  it("can move the cursor off the installed model onto recommended/more entries", { timeout: 15_000 }, async () => {
    ...
    stdin.write(DOWN);
    const afterDown = await waitForFrame(lastFrame, (f) => f.includes("› qwen2.5:7b"));
    ...
    stdin.write(DOWN);
    await waitForFrame(lastFrame, (f) => f.includes("› qwen2.5:14b"));

    unmount();
  });

  it("defaults to the first installed local model when the model step opens", { timeout: 15_000 }, async () => {
    const { lastFrame, stdin, unmount } = renderWizard();
    await waitForFrame(lastFrame, (f) => f.includes("1 / 3"));
    stdin.write(ENTER);
    await waitForFrame(lastFrame, (f) => f.includes("2 / 3"));
  ...
  it("shows installed and recommended group headers on the model step", { timeout: 15_000 }, async () => {
    const { lastFrame, stdin, unmount } = renderWizard();
    await waitForFrame(lastFrame, (f) => f.includes("1 / 3"));
    stdin.write(ENTER);
    await waitForFrame(lastFrame, (f) => f.includes("2 / 3"));
